...
if args.local_rank < args.num_gpus :
    ...
    #I cannot find the path to model. wenhao
    ...
    while True:
        ...
        if my_model_state[0] == -1:
            ...
            model.load_state_dict(torch.load(path_saver, map_location=torch.device('cuda:0'))) #I cannot find the path to model. wenhao
            continue
        my_mfgs = [None]
        ...
        if my_model_state[0] == 0:
            ...
            if mailbox is not None:
                with torch.no_grad():
                    ...
                    mailbox.update_memory(model.module.memory_updater.last_updated_nid, model.module.memory_updater.last_updated_memory, root_nodes, model.module.memory_updater.last_updated_ts) #adding root_nodes wenhao
                    if memory_param['deliver_to'] == 'neighbors':
    ...
    print('++++++++++++++++++++', hashlib.md5(str(torch.load(args.model, map_location=torch.device('cpu'))).encode('utf-8')).hexdigest() + '.pt')
