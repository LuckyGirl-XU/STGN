...
class Lorentzian(Manifold):
    ...
    def expmap0(self, p, c):
        return self.exp_map_zero(p, c)
    ...
    def mobius_add(self, x, y, c):  
        u = self.log_map_zero(y, c)
        v = self.ptransp0(x, u, c)
        return self.exp_map_x(v, x, c)   
    """
    def mobius_matvec(self, m, x, c):
        u = self.logmap0(x, c)
        mu = u @ m.transpose(-1, -2)
        mx = self.normalize_tangent_zero(mu, c)
   ...
   """
    def mobius_matvec(self, m, x, c):
        return self.matvec_regular(m, x, 0, c)

    def matvec_regular(self, m, x, b, c, use_bias = False):
        d = x.size(1) - 1
