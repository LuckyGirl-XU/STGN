...
def get_lr_scheduler(train_param, optimizer):
	if train_param['lr_scheduler'] == 'exponential':
		return optim.lr_scheduler.ExponentialLR(optimizer, gamma=args.lr_gamma)
	elif train_param['lr_scheduler'] == 'cosine':
		return optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=30, eta_min=0)
	elif train_param['lr_scheduler'] == 'cycle':
		return optim.lr_scheduler.CyclicLR(optimizer, 0, max_lr=args.lr, step_size_up=20, cycle_momentum=False)
	elif train_param['lr_scheduler'] == 'none':
		return NoneScheduler()

def get_optimizer(train_param, params):
	if train_param['optimizer'] == 'sgd':
		optimizer = optim.SGD(params, lr=train_param['lr'], weight_decay=train_param['weight_decay'])
	elif train_param['optimizer'] == 'adam':
		optimizer = optim.Adam(params, lr=train_param['lr'], weight_decay=train_param['weight_decay'])
	elif train_param['optimizer'] == 'amsgrad':
		optimizer = optim.Adam(params, lr=train_param['lr'], amsgrad=True, weight_decay=train_param['weight_decay'])
	return optimizer

def get_hyperbolic_optimizer(train_param, params):
    if train_param['hyper_optimizer'] == 'rsgd':
        optimizer = RiemannianSGD(
            args,
            params,
            lr=args.lr_hyperbolic,
        )
    elif train_param['hyper_optimizer'] == 'ramsgrad':
        optimizer = RiemannianAMSGrad(
            train_param,
            params,
            lr=train_param['lr_hyperbolic'],
        )
    else:
...
"""
def set_up_distributed_training_slurm(train_param):
    # get node and host
    ...
    th.cuda.set_device(train_param['device_id'])
...
def set_up_distributed_training_multi_gpu(args): 
    ...
    th.distributed.init_process_group(backend='nccl',
                                         init_method='env://')
"""
def save_model_weights(train_param, model, path):
	"""
	save model weights out to file
	"""
	if args.distributed_rank == 0:
		make_dir(path)
		th.save(model.state_dict(), os.path.join(path, train_param['name']))
