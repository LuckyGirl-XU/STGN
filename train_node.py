...
model = NodeClassificationModel(emb.shape[1], args.dim, labels.max() + 1, gnn_param).cuda()
...
with torch.no_grad():
    for emb, label in minibatch:
        ...
        if args.posneg:
            acc = average_precision_score(label.cpu(), pred.softmax(dim=1)[:, 1].cpu())
            roc = roc_auc_score(label.cpu(), pred.softmax(dim=1)[:, 1].cpu())
        else:
            acc = f1_score(label.cpu(), torch.argmax(pred, dim=1).cpu(), average="micro")
            roc = roc_auc_score(label.cpu(), torch.argmax(pred, dim=1).cpu())
        accs.append(acc)
        rocs.append(roc)
    acc = float(torch.tensor(accs).mean())
    roc = float(torch.tensor(rocs).mean())
print('Testing acc: {:.4f}'.format(acc))
print('Testing ROC: {:.4f}'.format(roc))